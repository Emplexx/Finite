...
class SubscriptionAdapterItem(
	...
	private val preferredCurrency: Currency,
	
	private val convertedAmount: ConvertedAmount,
	private val showUpcomingTime: Boolean,
	val palette: ItemColors,
	...
	private fun bindTimeLeft(binding: ItemSubscriptionBinding) {
		...
		// TODO this should be given to the ui model
...
//		TransitionManager.beginDelayedTransition(binding.root, changeClipBounds)
		...
		binding.textPrice.setText(buildString {
			...
			if (convertedAmount.isConverted) {
				append("≈ ")
			}
			...
			append(formatPrice(convertedAmount.amountMatchedToTimeframe))
		}, animate)
		
		binding.textPriceSubtitle.text = buildString {
			
			append(formatPrice(convertedAmount.amountOriginal))
			append(" ")
			
			val period = model.period.formatStringId
				?.let { binding.root.context.getString(it) }
				?: buildString {
					
					append("/ ")
					if (model.period.length > 1) append(model.period.length)
...
//			convertedAmount.amountMatchedToTimeframe != convertedAmount.amountOriginal
	...
	override fun getChangePayload(new: Item<*>): Any? {
		...
		return buildList {
			...
			if (showUpcomingTime != new.showUpcomingTime
				// TODO next payment should be part of the model
				|| model.findNextPayment() != new.model.findNextPayment()) add("TimeLeft")
